...
test('client apply sends pending reminder only', () => {
  ...
  try {
    ...
    // A completed-only flag without a running edge must NOT produce another reminder.
...
test('client apply sends completed-round reminder on running true -> false', () => {
  const originalWindow = globalThis.window
  const originalDocument = globalThis.document
  const navigatorDesc = Object.getOwnPropertyDescriptor(globalThis, 'navigator')
  const notificationDesc = Object.getOwnPropertyDescriptor(globalThis, 'Notification')
  ...
  const list = makeStore({
    phase: 'ready',
    ids: ['c'],
    byId: { c: { id: 'c', displayTitle: 'Completion', running: false } },
    current: undefined,
  })
  const dispose = apply({ sessions: { list, open() {} } })
  ...
  try {
    list.set({
      phase: 'ready',
      ids: ['c'],
      byId: { c: { id: 'c', displayTitle: 'Completion', running: true } },
      current: undefined,
    })
    assert.equal(document.body.children.length, 0, 'starting to run must not remind')
    list.set({
      phase: 'ready',
      ids: ['c'],
      byId: { c: { id: 'c', displayTitle: 'Completion', running: false } },
