...
/**
 * TEMP-DIAG（2026-09-24）：只读诊断打点，不改变任何提醒行为。
 * 记录写入 window.__dshNotifyDiag，用 CDP 读取以核对快照真实形状。
 * 定位完成后整体删除（搜索 TEMP-DIAG）。
 */
const DIAG_MAX = 400
function diag(msg, data) {
  if (typeof window === 'undefined') return
  try {
    const store = (window.__dshNotifyDiag = window.__dshNotifyDiag || [])
    store.push({ t: Date.now(), msg, data })
    if (store.length > DIAG_MAX) store.splice(0, store.length - DIAG_MAX)
  } catch {
    // 诊断失败绝不影响提醒主流程
  }
}

/** 把一份列表快照压成可读形状，用于核对字段名是否与运行时一致。 */
function diagShape(next) {
  if (next === null || next === undefined) return { snapshot: String(next) }
  const byId = next.byId
  const out = {
    phase: next.phase,
    topKeys: Object.keys(next).slice(0, 20),
    count: byId ? Object.keys(byId).length : null,
  }
  if (byId) {
    const ids = Object.keys(byId)
    const first = ids[0]
    if (first !== undefined) {
      const one = byId[first]
      out.sample = {
        id: first,
        keys: Object.keys(one || {}).slice(0, 25),
        running: one ? one.running : undefined,
        pendingInteraction: one ? one.pendingInteraction : undefined,
      }
    }
    out.runningIds = ids.filter((i) => byId[i] && byId[i].running === true).slice(0, 10)
    out.pendingIds = ids.filter((i) => byId[i] && byId[i].pendingInteraction !== undefined).slice(0, 10)
  }
  return out
}
 ...
 */
function shouldDiffList(prev, next) {
 ...
 */
function diffSummaries(prev, next, emit) {
 ...
 * @param emit - receives `{ kind, sessionId }` reminder events.
 */
function diffCompletions(next, prevRunning, emit) {
...
function createNotifier(options = {}) {
  ...
  function systemNotification(event, title, body) {
    if (typeof window === 'undefined') return false
    // TermuxLauncher embedded WebView exposes a native notification bridge.
    // Prefer it over the Web Notification API because WebView/Android
    // does not reliably support the browser Notification constructor.
    const bridge = window.themeBridge
    if (bridge && typeof bridge.notify === 'function') {
      try {
        bridge.notify(title, body, `dsh-notify-${event.sessionId}`)
        return true
      } catch {
        // Fall through to the web Notification path.
      }
    }
  ...
  function notify(event) {
    diag('notify.called', { started, kind: event && event.kind, sessionId: event && event.sessionId })
    if (!started) return
    const now = Date.now()
    if (now - lastNotifyAt < REMINDER_THROTTLE_MS) {
      diag('notify.throttled', { kind: event && event.kind, gap: now - lastNotifyAt })
      return
    }
    lastNotifyAt = now
    ...
    if (mobile) {
      ...
      diag('notify.mobileBranch', {
        systemShown,
        hasBridge: !!(typeof window !== 'undefined' && window.themeBridge && typeof window.themeBridge.notify === 'function'),
        title,
      })
      if (!systemShown) beep()
...
function apply(ctx) {
  ...
  diag('apply.entered', {
    ...
    hasList: !!(sessions && sessions.list),
    hasSubscribe: !!(sessions && sessions.list && typeof sessions.list.subscribe === 'function'),
    hasGetSnapshot: !!(sessions && sessions.list && typeof sessions.list.getSnapshot === 'function'),
    ctxKeys: ctx ? Object.keys(ctx).slice(0, 25) : null,
  })
  ...
  function onListChange() {
    ...
    diag('onListChange.snapshot', { shape: diagShape(next), ms: Date.now() - snapshotStart })
    ...
    diag('onListChange.branch', {
      prevPhase: lastList && lastList.phase,
      nextPhase: next && next.phase,
      firstReady,
      shouldDiff: shouldDiffList(lastList, next),
    })
    if (firstReady) {
      ...
      const emit = (event) => {
        if (event.kind === 'completed' && isUserStopped(event.sessionId)) {
          diag('emit.suppressedAsUserStopped', { sessionId: event.sessionId })
          return
        }
        const summary = next.byId[event.sessionId]
        const displayTitle = summary?.displayTitle ?? event.sessionId
        diag('emit.dispatch', { kind: event.kind, sessionId: event.sessionId, displayTitle })
        notifier.notify({
          ...event,
          title: displayTitle,
        })
      }
      const prevRunningCopy = new Map(prevRunning)
      diffSummaries(lastList, next, emit)
      diffCompletions(next, prevRunning, emit)
      diag('diff.done', {
        prevRunning: [...prevRunningCopy.entries()].slice(0, 10).map(([k, v]) => [k, v]),
        nextRunning: Object.keys(next.byId).slice(0, 10).map((i) => [i, next.byId[i] && next.byId[i].running === true]),
      })
    } else {
