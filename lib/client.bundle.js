...
function apply(ctx) {
  ...
  function startupPendingReminder(next) {
    ...
    const pendingIds = Object.values(next.byId)
      .filter((summary) => summary.pendingInteraction !== undefined)
      .map((summary) => summary.id)
    if (pendingIds.length === 0) return
    const first = next.byId[pendingIds[0]]
    const displayTitle = first?.displayTitle ?? pendingIds[0]
    notifier.notify({
      kind: 'pending',
      sessionId: pendingIds[0],
      title: pendingIds.length > 1 ? 'DSH 有会话需要处理' : displayTitle,
      message: pendingIds.length > 1
        ? `有 ${pendingIds.length} 个会话正在等待你处理/填写。`
